...
# q_adj * (E_real[t, s1] - E_forecast[t] * q[1]) - (E_forecast[t] * q[2]) <--- Gør det ikke-convext, hvis q ikke er fixed
...
@constraint(elec_model, adjustment_parameter[t in periods, s1 in scenarios1], E_adj[t, s1] == q_adj[1]*(E_real[t][s1] - E_forecast[t] * q[1]) - (E_forecast[t] * q[2]) + q_adj[2]*lambda_F[t]) 
...
# q: [0.8333333333333334, 0.01574658166666672]
# q_adj: [0.17600709884463492, 0.0940660147814567]
