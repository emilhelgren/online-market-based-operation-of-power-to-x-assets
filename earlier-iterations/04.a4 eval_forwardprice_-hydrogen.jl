...
@objective(elec_model, Max, 
    ...
    sum(
        ...
        + lambda_F[t]*0 * E_sold[t]
        - lambda_F[t]*10 * E_bought[t]
    for t in periods)
    )
...
data = [lambda_F, value.(forward_bid), value.(E_sold), value.(E_bought)]
names = ["Forward price", "Forward bid", "Power sold", "Power bought"]
filename = "RES_04.a4"
