...
data = [lambda_F, value.(forward_bid), value.(E_sold), value.(E_bought), value.(E_adj), obj_values]
names = ["Forward price", "Forward bid", "Power sold", "Power bought", "hydrogen produced", "Objective values"]
filename = "RES_04.a7"
easy_export(data, names, filename)


print("\n\nTjek lige om der er negative priser:")
for i in collect(1:length(lambda_F))
    if (lambda_F[i] < 0)
        print("\nPrisen er negative på index $i")
    end
end
