...
for t in eachindex(periods)
    act_obj = lambda_F[t] * value(forward_bid[t]) + lambda_H * value(E_adj[t]) + lambda_UP[t] * value(E_sold[t]) - lambda_DW[t] * value(E_bought[t])
...
data = [lambda_F, value.(forward_bid), value.(E_sold), value.(E_bought), value.(E_adj), obj_values, lambda_DW, lambda_UP, E_real, E_forecast]
names = ["Forward price", "Forward bid", "Power sold", "Power bought", "hydrogen produced", "Objective values", "lambda_DW", "lambda_UP", "E_real", "E_forecast"]
filename = "RES_04.b2"
