...
@objective(elec_model, Max, 
    ...
    sum(
        ...
        + lambda_H * E_adj[t]
        + 1/10 * sum(
            lambda_UP_s[i][t] * E_sold[t]
            - lambda_DW_s[i][t] * E_bought[t]
        for i in 1:10)
    for t in periods)
    )
...
print("\n\n\nONLY LAST TIMESTEP; ALL SCENARIOS")
for t in collect(1:10)
    print("\n\nFor period 10:")
    print("\n Realized production: $(E_real[10])")
    print("\n Forecasted production: $(value(E_forecast[10]))")
    print("\n Forward price: $(lambda_F[10])")
    print("\n Calculated bid: $(value(E_forecast[10]*q_F[1] + q_F[2]))")
    print("\n Forward bid actual: $(value(forward_bid[10]))")
    # print("\n Hydrogen consumption: $(value(E_forecast[t]*q_H[1] + q_H[2]))")
    print("\n Adjustment: $(value(E_adj[10]))")
    # print("\n Absolute value adjustment: $(value(E_adj_abs[s]))")
    # print("\n Are we buying?: $(value(b[s]))")
    print("\n Buying price: $(sum(lambda_DW_s[t][10]))")
    print("\n Selling price: $(sum(lambda_UP_s[t][10]))")

    print("\n Bought: $(value(E_bought[10]))")
    print("\n Sold: $(value(E_sold[10]))")
end
