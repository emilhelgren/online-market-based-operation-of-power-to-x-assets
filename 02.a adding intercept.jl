...
@objective(elec_model, Max, 
    ...
    sum(
        ...
        + lambda_H * (E_forecast[t]*q_H[1] + q_H[2] + E_adj[t])
...
@constraint(elec_model, adj_capacity_up[t in periods], E_adj[t] <= max_elec_capacity - (E_forecast[t] * q_H[1] + q_H[2]))
@constraint(elec_model, adj_capacity_dw[t in periods], -(E_forecast[t] * q_H[1] + q_H[2]) <= E_adj[t])
...
@constraint(elec_model, trade[t in periods], E_real[t] - (E_forecast[t] * q_F[1] + q_F[2]) - (E_forecast[t] * q_H[1] + q_H[2]) - E_adj[t] == E_T[t])
...
# 267.14571948780673    # No intercept
# 267.1457195757495     # With intercept